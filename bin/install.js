...
function checkDeps() {
  ...
  try { execSync("which git", { stdio: "ignore" }); } catch { missing.push("git"); }
  return missing;
}

function uninstall() {
  console.log();
  console.log(`  ${blue}nischal-statusline Uninstaller${reset}`);
...
function run() {
  ...
  console.log();
  console.log(`  ${blue}nischal-statusline Installer${reset}`);
  console.log(`  ${dim}────────────────────────────${reset}`);
  console.log();
  ...
  if (missing.length > 0) {
    ...
    if (missing.includes("jq")) log(`  ${dim}brew install jq${reset}`);
    process.exit(1);
  }
  success("Dependencies found (jq, git)");
